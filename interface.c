...
DEFINE_TREE(username_peer_pair, struct username_peer_pair *,
            username_peer_pair_cmp, NULL)
...
void fail_interface(struct tgl_state *TLS, struct in_ev *ev, int error_code,
                    const char *format, ...)
...
void socket_answer_start(void) { socket_answer_pos = 0; }

static void socket_answer_add_printf(const char *format, ...)
...
void socket_answer_add_printf(const char *format, ...) {
  ...
  va_start(ap, format);
  socket_answer_pos +=
      vsnprintf(socket_answer + socket_answer_pos,
                SOCKET_ANSWER_MAX_SIZE - socket_answer_pos, format, ap);
  va_end(ap);
  if (socket_answer_pos > SOCKET_ANSWER_MAX_SIZE) {
    socket_answer_pos = -1;
  }
}
...
#define mprint_end(ev, ...)                                                    \
  if (!ev) {                                                                   \
    print_end(__VA_ARGS__);                                                    \
  } else {                                                                     \
    socket_answer_end(ev);                                                     \
  }

#define mpush_color(ev, ...)                                                   \
  if (!ev) {                                                                   \
    push_color(__VA_ARGS__);                                                   \
  }

#define mpop_color(ev, ...)                                                    \
  if (!ev) {                                                                   \
    pop_color(__VA_ARGS__);                                                    \
  }
...
void next_token_end(void) {
  ...
  if (*line_ptr && *line_ptr != '"' && *line_ptr != '\'') {
    ...
    while (((unsigned char)cur_token[cur_token_len - 1]) <= ' ' &&
           cur_token_len >= 0) {
      cur_token_len--;
    }
...
tgl_peer_id_t parse_input_peer_id(const char *s, int l, int mask) {
  ...
  if (*s == '@') {
    ...
    struct username_peer_pair *p =
        tree_lookup_username_peer_pair(username_peer_pair, (void *)&tmp);
    tfree_str(tmp);
  ...
  const char *ss[] = {"user#id",    "user#",          "chat#id",
                      "chat#",      "secret_chat#id", "secret_chat#",
                      "channel#id", "channel#"};
  int tt[] = {TGL_PEER_USER,    TGL_PEER_USER,      TGL_PEER_CHAT,
              TGL_PEER_CHAT,    TGL_PEER_ENCR_CHAT, TGL_PEER_ENCR_CHAT,
              TGL_PEER_CHANNEL, TGL_PEER_CHANNEL};
...
char *get_default_prompt(void) {
  ...
  if (in_chat_mode) {
    ...
    l += snprintf(buf + l, 999 - l, COLOR_RED "%.*s " COLOR_NORMAL, 100,
                  U->print_name);
  }
  if (TLS->unread_messages || TLS->cur_uploading_bytes ||
      TLS->cur_downloading_bytes) {
    ...
    if (TLS->cur_uploading_bytes) {
      ...
      ok = 1;
      l += snprintf(buf + l, 999 - l, "%lld%%Up",
                    100 * TLS->cur_uploaded_bytes / TLS->cur_uploading_bytes);
    }
    if (TLS->cur_downloading_bytes) {
      ...
      ok = 1;
      l += snprintf(buf + l, 999 - l, "%lld%%Down",
                    100 * TLS->cur_downloaded_bytes /
                        TLS->cur_downloading_bytes);
    }
...
char *complete_none(const char *text, int state) { return 0; }
...
char *modifiers[] = {"[offline]", "[enable_preview]", "[disable_preview]",
                     "[html]",    "[reply=",          0};

char *in_chat_commands[] = {"/exit", "/quit", "/history", "/read", 0};
...
struct command {
  ...
  void (*fun)(struct command *command, int arg_num, struct arg args[],
              struct in_ev *ev);
  char *desc;
  void *arg;
};
...
void print_user_list_gw(struct tgl_state *TLS, void *extra, int success,
                        int num, struct tgl_user *UL[]);
void print_msg_list_gw(struct tgl_state *TLS, void *extra, int success, int num,
                       struct tgl_message *ML[]);
void print_msg_list_history_gw(struct tgl_state *TLS, void *extra, int success,
                               int num, struct tgl_message *ML[]);
void print_msg_list_success_gw(struct tgl_state *TLS, void *extra, int success,
                               int num, struct tgl_message *ML[]);
void print_dialog_list_gw(struct tgl_state *TLS, void *extra, int success,
                          int size, tgl_peer_id_t peers[],
                          tgl_message_id_t *last_msg_id[], int unread_count[]);
void print_chat_info_gw(struct tgl_state *TLS, void *extra, int success,
                        struct tgl_chat *C);
void print_channel_info_gw(struct tgl_state *TLS, void *extra, int success,
                           struct tgl_channel *C);
void print_user_info_gw(struct tgl_state *TLS, void *extra, int success,
                        struct tgl_user *C);
void print_filename_gw(struct tgl_state *TLS, void *extra, int success,
                       const char *name);
void print_string_gw(struct tgl_state *TLS, void *extra, int success,
                     const char *name);
void open_filename_gw(struct tgl_state *TLS, void *extra, int success,
                      const char *name);
void print_secret_chat_gw(struct tgl_state *TLS, void *extra, int success,
                          struct tgl_secret_chat *E);
void print_card_gw(struct tgl_state *TLS, void *extra, int success, int size,
                   int *card);
void print_user_gw(struct tgl_state *TLS, void *extra, int success,
                   struct tgl_user *U);
void print_peer_gw(struct tgl_state *TLS, void *extra, int success,
                   tgl_peer_t *U);
void print_msg_gw(struct tgl_state *TLS, void *extra, int success,
                  struct tgl_message *M);
void print_msg_success_gw(struct tgl_state *TLS, void *extra, int success,
                          struct tgl_message *M);
void print_encr_chat_success_gw(struct tgl_state *TLS, void *extra, int success,
                                struct tgl_secret_chat *E);
;
...
/* {{{ client methods */
void do_help(struct command *command, int arg_num, struct arg args[],
             struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    mprint_start(ev);
...
void do_get_terms_of_service(struct command *command, int arg_num,
                             struct arg args[], struct in_ev *ev) {
  assert(!arg_num);
  if (ev) {
    ev->refcnt++;
  }
...
void do_stats(struct command *command, int arg_num, struct arg args[],
              struct in_ev *ev) {
  assert(!arg_num);
...
void do_show_license(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(!arg_num);
  static char *b =
#include "LICENSE.h"
...
void do_quit(struct command *command, int arg_num, struct arg args[],
             struct in_ev *ev) {
  if (daemonize) {
...
void do_safe_quit(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  if (daemonize) {
    event_incoming(ev->bev, BEV_EVENT_EOF, ev);
  }
...
void do_set(struct command *command, int arg_num, struct arg args[],
            struct in_ev *ev) {
  int num = args[1].num;
...
void do_chat_with_peer(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  if (!ev) {
    in_chat_mode = 1;
...
void do_main_session(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
...
void do_version(struct command *command, int arg_num, struct arg args[],
                struct in_ev *ev) {
  assert(!arg_num);
  if (ev) {
    mprint_start(ev);
...
#define ARG2STR_DEF(n, def)                                                    \
  args[n].str ? args[n].str : def,                                             \
      args[n].str ? strlen(args[n].str) : strlen(def)
#define ARG2STR(n) args[n].str, args[n].str ? strlen(args[n].str) : 0

/* {{{ WORK WITH ACCOUNT */

void do_set_password(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_msg(struct command *command, int arg_num, struct arg args[],
            struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  ...
  vlogprintf(E_DEBUG, "reply_id=%d, disable=%d\n", reply_id,
             disable_msg_preview);
  tgl_do_send_message(TLS, args[0].peer_id, ARG2STR(1),
                      TGL_SEND_MSG_FLAG_REPLY(reply_id) | disable_msg_preview |
                          do_html,
                      NULL, print_msg_success_gw, ev);
  if (auto_mark_read)
    tgl_do_mark_read(TLS, args[0].peer_id, 0, 0);
}

void do_post(struct command *command, int arg_num, struct arg args[],
             struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  ...
  tgl_do_send_message(TLS, args[0].peer_id, ARG2STR(1),
                      TGL_SEND_MSG_FLAG_REPLY(reply_id) | disable_msg_preview |
                          TGLMF_POST_AS_CHANNEL | do_html,
                      NULL, print_msg_success_gw, ev);
}

void do_msg_kbd(struct command *command, int arg_num, struct arg args[],
                struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  ...
  struct tl_ds_reply_markup *DS_RM =
      fetch_ds_type_reply_markup(TYPE_TO_PARAM(reply_markup));
  assert(DS_RM);

  tgl_do_send_message(TLS, args[0].peer_id, ARG2STR(2),
                      TGL_SEND_MSG_FLAG_REPLY(reply_id) | disable_msg_preview |
                          do_html,
                      DS_RM, print_msg_success_gw, ev);

  free_ds_type_reply_markup(DS_RM, TYPE_TO_PARAM(reply_markup));
}

void do_reply(struct command *command, int arg_num, struct arg args[],
              struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_reply_message(TLS, &args[0].msg_id, ARG2STR(1),
                       disable_msg_preview | do_html, print_msg_success_gw, ev);
  ...
  if (auto_mark_read) {
    ...
    if (M) {
      ...
      tgl_peer_id_t id = (M->to_id.peer_type == TGL_PEER_CHAT ||
                          M->to_id.peer_id != TLS->our_id.peer_id)
                             ? M->to_id
                             : M->from_id;
...
void do_send_text(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_send_text(TLS, args[0].peer_id, args[1].str,
                   TGL_SEND_MSG_FLAG_REPLY(reply_id) | disable_msg_preview |
                       do_html,
                   print_msg_success_gw, ev);
}

void do_post_text(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
...
void do_reply_text(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_reply_text(TLS, &args[0].msg_id, args[1].str,
                    disable_msg_preview | do_html, print_msg_success_gw, ev);
}

static void _do_send_file(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev,
  ...
  tgl_do_send_document(
      TLS, args[0].peer_id, args[1].str, arg_num == 2 ? NULL : args[2].str,
      arg_num == 2 ? 0 : strlen(args[2].str),
      flags | TGL_SEND_MSG_FLAG_REPLY(reply_id), print_msg_success_gw, ev);
}

void do_send_photo(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_PHOTO);
}

void do_send_file(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_AUTO);
}

void do_send_audio(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_AUDIO);
}

void do_send_video(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_VIDEO);
}

void do_send_document(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev, 0);
}

void do_post_photo(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev,
                TGL_SEND_MSG_FLAG_DOCUMENT_PHOTO | TGLMF_POST_AS_CHANNEL);
}

void do_post_file(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev,
                TGL_SEND_MSG_FLAG_DOCUMENT_AUTO | TGLMF_POST_AS_CHANNEL);
}

void do_post_audio(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev,
                TGL_SEND_MSG_FLAG_DOCUMENT_AUDIO | TGLMF_POST_AS_CHANNEL);
}

void do_post_video(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev,
                TGL_SEND_MSG_FLAG_DOCUMENT_VIDEO | TGLMF_POST_AS_CHANNEL);
}

void do_post_document(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  _do_send_file(command, arg_num, args, ev, TGLMF_POST_AS_CHANNEL);
}

void _do_reply_file(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev, unsigned long long flags) {
  assert(arg_num >= 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_reply_document(
      TLS, &args[0].msg_id, args[1].str, arg_num == 2 ? NULL : args[2].str,
      arg_num == 2 ? 0 : strlen(args[2].str), flags, print_msg_success_gw, ev);
}

void do_reply_photo(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  _do_reply_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_PHOTO);
}

void do_reply_file(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  _do_reply_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_AUTO);
}

void do_reply_audio(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  _do_reply_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_AUDIO);
}

void do_reply_video(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  _do_reply_file(command, arg_num, args, ev, TGL_SEND_MSG_FLAG_DOCUMENT_VIDEO);
}

void do_reply_document(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  _do_reply_file(command, arg_num, args, ev, 0);
}

void do_fwd(struct command *command, int arg_num, struct arg args[],
            struct in_ev *ev) {
  assert(arg_num >= 2);
  if (ev) {
    ev->refcnt++;
  ...
  assert(arg_num <= 1000);
  // if (arg_num == 2) {
  //  tgl_do_forward_message (TLS, args[0].P->id, &args[1].msg_id, 0,
  //  print_msg_success_gw, ev); if (auto_mark_read)
  ...
  tgl_do_forward_messages(TLS, args[0].peer_id, arg_num - 1, (void *)list, 0,
                          print_msg_list_success_gw, ev);
  if (auto_mark_read)
    tgl_do_mark_read(TLS, args[0].peer_id, 0, 0);
  //}
}

void do_fwd_media(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_forward_media(TLS, args[0].peer_id, &args[1].msg_id, 0,
                       print_msg_success_gw, ev);
}

void do_send_contact(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 4);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_send_contact(TLS, args[0].peer_id, ARG2STR(1), ARG2STR(2), ARG2STR(3),
                      TGL_SEND_MSG_FLAG_REPLY(reply_id), print_msg_success_gw,
                      ev);
}

void do_reply_contact(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 4);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_reply_contact(TLS, &args[0].msg_id, ARG2STR(1), ARG2STR(2), ARG2STR(3),
                       0, print_msg_success_gw, ev);
}

void do_send_location(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_send_location(TLS, args[0].peer_id, args[1].dval, args[2].dval,
                       TGL_SEND_MSG_FLAG_REPLY(reply_id), print_msg_success_gw,
                       ev);
}

void do_post_location(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  ...
  tgl_do_send_location(TLS, args[0].peer_id, args[1].dval, args[2].dval,
                       TGL_SEND_MSG_FLAG_REPLY(reply_id) |
                           TGLMF_POST_AS_CHANNEL,
                       print_msg_success_gw, ev);
}

void do_reply_location(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_reply_location(TLS, &args[0].msg_id, args[1].dval, args[2].dval, 0,
                        print_msg_success_gw, ev);
}

void do_broadcast(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  ...
  tgl_do_send_broadcast(TLS, arg_num - 1, ids, args[arg_num - 1].str,
                        strlen(args[arg_num - 1].str),
                        disable_msg_preview | do_html,
                        print_msg_list_success_gw, ev);
}
...
void do_get_self(struct command *command, int arg_num, struct arg args[],
                 struct in_ev *ev) {
  if (ev) {
    ev->refcnt++;
  }
...
void do_set_profile_photo(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_set_profile_name(struct command *command, int arg_num,
                         struct arg args[], struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
...
void do_set_username(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_set_phone_number(struct command *command, int arg_num,
                         struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_status_online(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(!arg_num);
  if (ev) {
    ev->refcnt++;
...
void do_status_offline(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  assert(!arg_num);
  if (ev) {
    ev->refcnt++;
...
void do_export_card(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(!arg_num);
  if (ev) {
    ev->refcnt++;
...
void do_chat_set_photo(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_set_chat_photo(TLS, args[0].peer_id, args[1].str, print_success_gw,
                        ev);
}

void do_rename_chat(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
...
void do_chat_info(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_get_chat_info(TLS, args[0].peer_id, offline_mode, print_chat_info_gw,
                       ev);
}

void do_channel_info(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_get_channel_info(TLS, args[0].peer_id, offline_mode,
                          print_channel_info_gw, ev);
}

void do_chat_add_user(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_add_user_to_chat(TLS, args[0].peer_id, args[1].peer_id,
                          args[2].num != NOT_FOUND ? args[2].num : 100,
                          print_success_gw, ev);
}

void do_chat_del_user(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_del_user_from_chat(TLS, args[0].peer_id, args[1].peer_id,
                            print_success_gw, ev);
}

void do_create_group_chat(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev) {
  assert(arg_num >= 1 && arg_num <= 1000);
  static tgl_peer_id_t ids[1000];
  int i;
  ...
  tgl_do_create_group_chat(TLS, arg_num - 1, ids, ARG2STR(0), print_success_gw,
                           ev);
}

void do_export_chat_link(struct command *command, int arg_num,
                         struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_import_chat_link(struct command *command, int arg_num,
                         struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_channel_invite(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_channel_invite_user(TLS, args[0].peer_id, args[1].peer_id,
                             print_success_gw, ev);
}

void do_channel_kick(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_channel_kick_user(TLS, args[0].peer_id, args[1].peer_id,
                           print_success_gw, ev);
}

void do_channel_get_members(struct command *command, int arg_num,
                            struct arg args[], struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  ...
  tgl_do_channel_get_members(
      TLS, args[0].peer_id, args[1].num == NOT_FOUND ? 100 : args[1].num,
      args[2].num == NOT_FOUND ? 0 : args[2].num, 0, print_user_list_gw, ev);
}

void do_channel_get_admins(struct command *command, int arg_num,
                           struct arg args[], struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  ...
  tgl_do_channel_get_members(
      TLS, args[0].peer_id, args[1].num == NOT_FOUND ? 100 : args[1].num,
      args[2].num == NOT_FOUND ? 0 : args[2].num, 1, print_user_list_gw, ev);
}

void do_chat_upgrade(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_user_info(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_get_user_info(TLS, args[0].peer_id, offline_mode, print_user_info_gw,
                       ev);
}

void do_add_contact(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_add_contact(TLS, ARG2STR(0), ARG2STR(1), ARG2STR(2), 0,
                     print_user_list_gw, ev);
}

void do_rename_contact(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  assert(arg_num == 3);
  ...
  if (P && P->user.phone) {
    ...
    tgl_do_add_contact(TLS, P->user.phone, strlen(P->user.phone), args[1].str,
                       strlen(args[1].str), args[2].str, strlen(args[2].str), 0,
                       print_user_list_gw, ev);
  } else {
    if (ev) {
      ev->refcnt++;
...
void do_del_contact(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_import_card(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num == 1);
  char *s = args[0].str;
  int l = strlen(s);
...
void do_block_user(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_unblock_user(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_accept_secret_chat(struct command *command, int arg_num,
                           struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
  ...
  tgl_peer_t *P = tgl_peer_get(TLS, args[0].peer_id);
  if (P) {
    tgl_do_accept_encr_chat_request(TLS, &P->encr_chat,
                                    print_encr_chat_success_gw, ev);
  } else {
    print_success_gw(TLS, ev, 0);
  }
}

void do_set_ttl(struct command *command, int arg_num, struct arg args[],
                struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  ...
  if (P && P->encr_chat.state == sc_ok) {
    tgl_do_set_encr_chat_ttl(TLS, &P->encr_chat, args[1].num,
                             print_msg_success_gw, ev);
  } else {
    print_success_gw(TLS, ev, 0);
  }
...
void do_visualize_key(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 1);
...
void do_create_secret_chat(struct command *command, int arg_num,
                           struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_rename_channel(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
...
void do_channel_set_photo(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_set_channel_photo(TLS, args[0].peer_id, args[1].str, print_success_gw,
                           ev);
}

void do_channel_set_about(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_channel_set_about(TLS, args[0].peer_id, ARG2STR(1), print_success_gw,
                           ev);
}

void do_channel_set_admin(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_channel_set_admin(TLS, args[0].peer_id, args[1].peer_id, args[2].num,
                           print_success_gw, ev);
}

void do_channel_set_username(struct command *command, int arg_num,
                             struct arg args[], struct in_ev *ev) {
  assert(arg_num == 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_channel_set_username(TLS, args[0].peer_id, ARG2STR(1),
                              print_success_gw, ev);
}

void do_create_channel(struct command *command, int arg_num, struct arg args[],
                       struct in_ev *ev) {
  ...
  tgl_do_create_channel(TLS, arg_num - 2, ids, ARG2STR(0), ARG2STR(1), 1,
                        print_success_gw, ev);
}

void do_join_channel(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_leave_channel(struct command *command, int arg_num, struct arg args[],
                      struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_export_channel_link(struct command *command, int arg_num,
                            struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_dialog_list(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num <= 2);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_get_dialog_list(TLS, args[0].num != NOT_FOUND ? args[0].num : 100,
                         args[1].num != NOT_FOUND ? args[1].num : 0,
                         print_dialog_list_gw, ev);
}

void do_channel_list(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(arg_num <= 2);
  if (ev) {
    ev->refcnt++;
  ...
  tgl_do_get_channels_dialog_list(
      TLS, args[0].num != NOT_FOUND ? args[0].num : 100,
      args[1].num != NOT_FOUND ? args[1].num : 0, print_dialog_list_gw, ev);
}

void do_resolve_username(struct command *command, int arg_num,
                         struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_contact_search(TLS, args[0].str, strlen(args[0].str), print_peer_gw,
                        ev);
}

void do_contact_list(struct command *command, int arg_num, struct arg args[],
                     struct in_ev *ev) {
  assert(!arg_num);
  if (ev) {
    ev->refcnt++;
...
void do_mark_read(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_history(struct command *command, int arg_num, struct arg args[],
                struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_get_history(TLS, args[0].peer_id,
                     args[2].num != NOT_FOUND ? args[2].num : 0,
                     args[1].num != NOT_FOUND ? args[1].num : 40, offline_mode,
                     print_msg_list_history_gw, ev);
}

void print_fail(struct in_ev *ev);

void do_send_typing(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num == 2);
  ...
  if (args[1].num != NOT_FOUND) {
    ...
    status = (enum tgl_typing_status)args[1]
                 .num; // if the status parameter is given, and is in range.
  }
  if (ev) {
    ev->refcnt++;
  }
...
void do_send_typing_abort(struct command *command, int arg_num,
                          struct arg args[], struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_send_typing(TLS, args[0].peer_id, tgl_typing_cancel, print_success_gw,
                     ev);
}
...
#define DO_LOAD_PHOTO(tp, act, actf)                                           \
  void do_##act##_##tp(struct command *command, int arg_num,                   \
                       struct arg args[], struct in_ev *ev) {                  \
    assert(arg_num == 1);                                                      \
    if (ev) {                                                                  \
      ev->refcnt++;                                                            \
      vlogprintf(E_WARNING, "refcnt+\n");                                      \
    }                                                                          \
    struct tgl_message *M = tgl_message_get(TLS, &args[0].msg_id);             \
    if (M && !(M->flags & TGLMF_SERVICE)) {                                    \
      ...
      if (M->media.type == tgl_message_media_photo) {                          \
        ...
        actf(TLS, ev, 1, M->media.webpage->url);                               \
                 ...
                 M->media.type == tgl_message_media_venue) {                   \
        static char s[1000];                                                   \
        sprintf(s, "https://maps.google.com/?q=%.6lf,%.6lf",                   \
...
#define DO_LOAD_PHOTO_THUMB(tp, act, actf)                                     \
  void do_##act##_##tp##_thumb(struct command *command, int arg_num,           \
                               struct arg args[], struct in_ev *ev) {          \
    assert(arg_num == 1);                                                      \
    if (ev) {                                                                  \
      ev->refcnt++;                                                            \
...
void do_load_user_photo(struct command *command, int arg_num, struct arg args[],
                        struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_view_user_photo(struct command *command, int arg_num, struct arg args[],
                        struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_search(struct command *command, int arg_num, struct arg args[],
               struct in_ev *ev) {
  assert(arg_num == 6);
  int limit;
  if (args[1].num != NOT_FOUND) {
  ...
  tgl_do_msg_search(TLS, args[0].peer_id, from, to, limit, offset, args[5].str,
                    strlen(args[5].str), print_msg_list_gw, ev);
}

void do_delete_msg(struct command *command, int arg_num, struct arg args[],
                   struct in_ev *ev) {
  if (ev) {
    ev->refcnt++;
  }
...
void do_get_message(struct command *command, int arg_num, struct arg args[],
                    struct in_ev *ev) {
  assert(arg_num == 1);
  if (ev) {
    ev->refcnt++;
...
void do_start_bot(struct command *command, int arg_num, struct arg args[],
                  struct in_ev *ev) {
  assert(arg_num == 3);
  if (ev) {
    ev->refcnt++;
  }
  tgl_do_start_bot(TLS, args[0].peer_id, args[1].peer_id, ARG2STR(2),
                   print_success_gw, ev);
}
/* }}} */
...
void do_clear(struct command *command, int arg_num, struct arg args[],
              struct in_ev *ev) {
  logprintf("Do_clear\n");
...
struct command commands[MAX_COMMANDS_SIZE] = {
    {"accept_secret_chat",
     {ca_secret_chat, ca_none},
     do_accept_secret_chat,
     "accept_secret_chat <secret chat>\tAccepts secret chat. Only useful with "
     "-E option",
     NULL},
    {"add_contact",
     {ca_string, ca_string, ca_string, ca_none},
     do_add_contact,
     "add_contact <phone> <first name> <last name>\tTries to add user to "
     "contact list",
     NULL},
    {"block_user",
     {ca_user, ca_none},
     ...
     NULL},
    {"broadcast",
     {ca_user, ca_period, ca_string_end, ca_none},
     do_broadcast,
     "broadcast <user>+ <text>\tSends text to several users at once",
     NULL},
    {"channel_get_admins",
     {ca_channel, ca_number | ca_optional, ca_number | ca_optional, ca_none},
    ...
    {"channel_leave",
     ...
     NULL},
    {"channel_list",
     {ca_number | ca_optional, ca_number | ca_optional, ca_none},
     do_channel_list,
     "channel_list [limit=100] [offset=0]\tList of last channels",
     NULL},
    {"channel_set_about",
     {ca_channel, ca_string, ca_none},
     do_channel_set_about,
     "channel_set_about <channel> <about>\tSets channel about info.",
     NULL},
    {"channel_set_admin",
     {ca_channel, ca_user, ca_number, ca_none},
     do_channel_set_admin,
     "channel_set_admin <channel> <admin> <type>\tSets channel admin. 0 - not "
     "admin, 1 - moderator, 2 - editor",
     NULL},
    {"channel_set_username",
     {ca_channel, ca_string, ca_none},
     do_channel_set_username,
     "channel_set_username <channel> <username>\tSets channel username info.",
     NULL},
    {"channel_set_photo",
     {ca_channel, ca_file_name_end, ca_none},
     do_channel_set_photo,
     "channel_set_photo <channel> <filename>\tSets channel photo. Photo will "
     "be cropped to square",
     NULL},
    {"chat_add_user",
     {ca_chat, ca_user, ca_number | ca_optional, ca_none},
     do_chat_add_user,
     "chat_add_user <chat> <user> [msgs-to-forward]\tAdds user to chat. Sends "
     "him last msgs-to-forward message from this chat. Default 100",
     NULL},
    {"chat_del_user",
     {ca_chat, ca_user, ca_none},
     do_chat_del_user,
    ...
    {"chat_info",
     ...
     NULL},
    {"chat_set_photo",
     {ca_chat, ca_file_name_end, ca_none},
     do_chat_set_photo,
     "chat_set_photo <chat> <filename>\tSets chat photo. Photo will be cropped "
     "to square",
     NULL},
    {"chat_upgrade",
     {ca_chat, ca_none},
    ...
    {"chat_with_peer",
     {ca_peer, ca_none},
     do_chat_with_peer,
     "chat_with_peer <peer>\tInterface option. All input will be treated as "
     "messages to this peer. Type /quit to end this mode",
     NULL},
    {"clear",
     {ca_none},
     do_clear,
    ...
    {"contact_search",
     ...
     NULL},
    {"create_channel",
     {ca_string, ca_string, ca_user | ca_optional, ca_period, ca_none},
     do_create_channel,
     "create_channel <name> <about> <user>+\tCreates channel with users",
     NULL},
    {"create_group_chat",
     {ca_string, ca_user, ca_period, ca_none},
    ...
    {"export_card",
     {ca_none},
     do_export_card,
     "export_card\tPrints card that can be imported by another user with "
     "import_card method",
     NULL},
    {"export_channel_link",
     {ca_channel, ca_none},
     do_export_channel_link,
     "export_channel_link\tPrints channel link that can be used to join to "
     "channel",
     NULL},
    {"export_chat_link",
     {ca_chat, ca_none},
     ...
     NULL},
    {"fwd",
     {ca_peer, ca_msg_id, ca_period, ca_none},
     do_fwd,
     "fwd <peer> <msg-id>+\tForwards message to peer. Forward to secret chats "
     "is forbidden",
     NULL},
    {"fwd_media",
     {ca_peer, ca_msg_id, ca_none},
     do_fwd_media,
     "fwd_media <peer> <msg-id>\tForwards message media to peer. Forward to "
     "secret chats is forbidden. Result slightly differs from fwd",
     NULL},
    {"get_terms_of_service",
     {ca_none},
     do_get_terms_of_service,
    ...
    {"help",
     {ca_command | ca_optional, ca_none},
     do_help,
     "help [command]\tPrints this help",
     NULL},
    {"history",
     {ca_peer, ca_number | ca_optional, ca_number | ca_optional, ca_none},
     do_history,
     "history <peer> [limit] [offset]\tPrints messages with this peer (most "
     "recent message lower). Also marks messages as read",
     NULL},
    {"import_card",
     {ca_string, ca_none},
     do_import_card,
     "import_card <card>\tGets user by card and prints it name. You can then "
     "send messages to him as usual",
     NULL},
    {"import_chat_link",
     {ca_string, ca_none},
     do_import_chat_link,
    ...
    {"load_audio",
     {ca_msg_id, ca_none},
     do_load_audio,
     "load_audio <msg-id>\tDownloads file to downloads dirs. Prints file name "
     "after download end",
     NULL},
    {"load_channel_photo",
     {ca_channel, ca_none},
     do_load_user_photo,
     "load_channel_photo <channel>\tDownloads file to downloads dirs. Prints "
     "file name after download end",
     NULL},
    {"load_chat_photo",
     {ca_chat, ca_none},
     do_load_user_photo,
     "load_chat_photo <chat>\tDownloads file to downloads dirs. Prints file "
     "name after download end",
     NULL},
    {"load_document",
     {ca_msg_id, ca_none},
     do_load_document,
     "load_document <msg-id>\tDownloads file to downloads dirs. Prints file "
     "name after download end",
     NULL},
    {"load_document_thumb",
     {ca_msg_id, ca_none},
     do_load_document_thumb,
     "load_document_thumb <msg-id>\tDownloads file to downloads dirs. Prints "
     "file name after download end",
     NULL},
    {"load_file",
     {ca_msg_id, ca_none},
     do_load_file,
     "load_file <msg-id>\tDownloads file to downloads dirs. Prints file name "
     "after download end",
     NULL},
    {"load_file_thumb",
     {ca_msg_id, ca_none},
     do_load_file_thumb,
     "load_file_thumb <msg-id>\tDownloads file to downloads dirs. Prints file "
     "name after download end",
     NULL},
    {"load_photo",
     {ca_msg_id, ca_none},
     do_load_photo,
     "load_photo <msg-id>\tDownloads file to downloads dirs. Prints file name "
     "after download end",
     NULL},
    {"load_user_photo",
     {ca_user, ca_none},
     do_load_user_photo,
     "load_user_photo <user>\tDownloads file to downloads dirs. Prints file "
     "name after download end",
     NULL},
    {"load_video",
     {ca_msg_id, ca_none},
     do_load_video,
     "load_video <msg-id>\tDownloads file to downloads dirs. Prints file name "
     "after download end",
     NULL},
    {"load_video_thumb",
     {ca_msg_id, ca_none},
     do_load_video_thumb,
     "load_video_thumb <msg-id>\tDownloads file to downloads dirs. Prints file "
     "name after download end",
     NULL},
    {"main_session",
     {ca_none},
     do_main_session,
     "main_session\tSends updates to this connection (or terminal). Useful "
     "only with listening socket",
     NULL},
    {"mark_read",
     {ca_peer, ca_none},
     do_mark_read,
     ...
     NULL},
    {"msg",
     {ca_peer, ca_msg_string_end, ca_none},
     do_msg,
     "msg <peer> <text>\tSends text message to peer",
     NULL},
    {"msg_kbd",
     {ca_peer, ca_string, ca_msg_string_end, ca_none},
     do_msg_kbd,
    ...
    {"post_file",
     ...
     NULL},
    {"post_location",
     {ca_peer, ca_double, ca_double, ca_none},
     do_post_location,
     "post_location <peer> <latitude> <longitude>\tSends geo location",
     NULL},
    {"post_photo",
     {ca_peer, ca_file_name, ca_string_end | ca_optional, ca_none},
     do_post_photo,
     "post_photo <peer> <file> [caption]\tSends photo to peer",
     NULL},
    {"post_text",
     {ca_peer, ca_file_name_end, ca_none},
     do_post_text,
     "post_text <peer> <file>\tSends contents of text file as plain text "
     "message",
     NULL},
    {"post_video",
     {ca_peer, ca_file_name, ca_string_end | ca_optional, ca_none},
    ...
    {"rename_channel",
     {ca_channel, ca_string_end, ca_none},
     do_rename_channel,
     "rename_channel <channel> <new name>\tRenames channel",
     NULL},
    {"rename_chat",
     {ca_chat, ca_string_end, ca_none},
     do_rename_chat,
     "rename_chat <chat> <new name>\tRenames chat",
     NULL},
    {"rename_contact",
     {ca_user, ca_string, ca_string, ca_none},
     do_rename_contact,
     "rename_contact <user> <first name> <last name>\tRenames contact",
     NULL},
    {"reply",
     {ca_msg_id, ca_msg_string_end, ca_none},
     do_reply,
     "reply <msg-id> <text>\tSends text reply to message",
     NULL},
    {"reply_audio",
     {ca_msg_id, ca_file_name, ca_none},
     do_send_audio,
     "reply_audio <msg-id> <file>\tSends audio to peer",
     NULL},
    {"reply_contact",
     {ca_msg_id, ca_string, ca_string, ca_string, ca_none},
     do_reply_contact,
     "reply_contact <msg-id> <phone> <first-name> <last-name>\tSends contact "
     "(not necessary telegram user)",
     NULL},
    {"reply_document",
     {ca_msg_id, ca_file_name, ca_none},
     do_reply_document,
    ...
    {"reply_file",
     ...
     NULL},
    {"reply_location",
     {ca_msg_id, ca_double, ca_double, ca_none},
     do_reply_location,
     "reply_location <msg-id> <latitude> <longitude>\tSends geo location",
     NULL},
    {"reply_photo",
     {ca_msg_id, ca_file_name, ca_string_end | ca_optional, ca_none},
     do_reply_photo,
     "reply_photo <msg-id> <file> [caption]\tSends photo to peer",
     NULL},
    //{"reply_text", {ca_number, ca_file_name_end, ca_none}, do_reply_text,
    //"reply_text <msg-id> <file>\tSends contents of text file as plain text
    //message", NULL},
    {"reply_video",
     {ca_msg_id, ca_file_name, ca_none},
     do_reply_video,
    ...
    {"resolve_username",
     ...
     NULL},
    //{"restore_msg", {ca_number, ca_none}, do_restore_msg, "restore_msg
    //<msg-id>\tRestores message. Only available shortly (one hour?) after
    //deletion", NULL},
    {"safe_quit",
     {ca_none},
     do_safe_quit,
     ...
     NULL},
    {"search",
     {ca_peer | ca_optional, ca_number | ca_optional, ca_number | ca_optional,
      ca_number | ca_optional, ca_number | ca_optional, ca_string_end},
     do_search,
     "search [peer] [limit] [from] [to] [offset] pattern\tSearch for pattern "
     "in messages from date from to date to (unixtime) in messages with peer "
     "(if peer not present, in all messages)",
     NULL},
    //{"secret_chat_rekey", { ca_secret_chat, ca_none}, do_secret_chat_rekey,
    //"generate new key for active secret chat", NULL},
    {"send_audio",
     {ca_peer, ca_file_name, ca_string_end | ca_optional, ca_none},
     do_send_audio,
     ...
     NULL},
    {"send_contact",
     {ca_peer, ca_string, ca_string, ca_string, ca_none},
     do_send_contact,
     "send_contact <peer> <phone> <first-name> <last-name>\tSends contact (not "
     "necessary telegram user)",
     NULL},
    {"send_document",
     {ca_peer, ca_file_name, ca_string_end | ca_optional, ca_none},
     do_send_document,
    ...
    {"send_text",
     {ca_peer, ca_file_name_end, ca_none},
     do_send_text,
     "send_text <peer> <file>\tSends contents of text file as plain text "
     "message",
     NULL},
    {"send_typing",
     {ca_peer, ca_number | ca_optional, ca_none},
     do_send_typing,
     "send_typing <peer> [status]\tSends typing notification. You can supply a "
     "custom status (range 0-10): none, typing, cancel, record video, upload "
     "video, record audio, upload audio, upload photo, upload document, geo, "
     "choose contact.",
     NULL},
    {"send_typing_abort",
     {ca_peer, ca_none},
     do_send_typing_abort,
    ...
    {"send_video",
     ...
     NULL},
    {"set",
     {ca_string, ca_number, ca_none},
     do_set,
     "set <param> <value>\tSets value of param. Currently available: "
     "log_level, debug_verbosity, alarm, auto_mark_read, msg_num",
     NULL},
    {"set_password",
     {ca_string | ca_optional, ca_none},
     do_set_password,
     "set_password <hint>\tSets password",
     NULL},
    {"set_profile_name",
     {ca_string, ca_string, ca_none},
     do_set_profile_name,
     "set_profile_name <first-name> <last-name>\tSets profile name.",
     NULL},
    {"set_profile_photo",
     {ca_file_name_end, ca_none},
     do_set_profile_photo,
     "set_profile_photo <filename>\tSets profile photo. Photo will be cropped "
     "to square",
     NULL},
    {"set_ttl",
     {ca_secret_chat, ca_number, ca_none},
     do_set_ttl,
    ...
    {"show_license",
     ...
     NULL},
    {"start_bot",
     {ca_user, ca_chat, ca_string, ca_none},
     do_start_bot,
    ...
    {"view_audio",
     {ca_msg_id, ca_none},
     do_open_audio,
     "view_audio <msg-id>\tDownloads file to downloads dirs. Then tries to "
     "open it with system default action",
     NULL},
    {"view_channel_photo",
     {ca_channel, ca_none},
     do_view_user_photo,
     "view_channel_photo <channel>\tDownloads file to downloads dirs. Then "
     "tries to open it with system default action",
     NULL},
    {"view_chat_photo",
     {ca_chat, ca_none},
     do_view_user_photo,
     "view_chat_photo <chat>\tDownloads file to downloads dirs. Then tries to "
     "open it with system default action",
     NULL},
    {"view_document",
     {ca_msg_id, ca_none},
     do_open_document,
     "view_document <msg-id>\tDownloads file to downloads dirs. Then tries to "
     "open it with system default action",
     NULL},
    {"view_document_thumb",
     {ca_msg_id, ca_none},
     do_open_document_thumb,
     "view_document_thumb <msg-id>\tDownloads file to downloads dirs. Then "
     "tries to open it with system default action",
     NULL},
    {"view_file",
     {ca_msg_id, ca_none},
     do_open_file,
     "view_file <msg-id>\tDownloads file to downloads dirs. Then tries to open "
     "it with system default action",
     NULL},
    {"view_file_thumb",
     {ca_msg_id, ca_none},
     do_open_file_thumb,
     "view_file_thumb <msg-id>\tDownloads file to downloads dirs. Then tries "
     "to open it with system default action",
     NULL},
    {"view_photo",
     {ca_msg_id, ca_none},
     do_open_photo,
     "view_photo <msg-id>\tDownloads file to downloads dirs. Then tries to "
     "open it with system default action",
     NULL},
    {"view_user_photo",
     {ca_user, ca_none},
     do_view_user_photo,
     "view_user_photo <user>\tDownloads file to downloads dirs. Then tries to "
     "open it with system default action",
     NULL},
    {"view_video",
     {ca_msg_id, ca_none},
     do_open_video,
     "view_video <msg-id>\tDownloads file to downloads dirs. Then tries to "
     "open it with system default action",
     NULL},
    {"view_video_thumb",
     {ca_msg_id, ca_none},
     do_open_video_thumb,
     "view_video_thumb <msg-id>\tDownloads file to downloads dirs. Then tries "
     "to open it with system default action",
     NULL},
    {"view",
     {ca_msg_id, ca_none},
    ...
    {"visualize_key",
     {ca_secret_chat, ca_none},
     do_visualize_key,
     "visualize_key <secret chat>\tPrints visualization of encryption key "
     "(first 16 bytes sha1 of it in fact)",
     NULL}};
...
enum command_argument get_complete_mode(void) {
  ...
  while (1) {
    ...
    char *save = line_ptr;
    next_token();
    if (op == ca_user || op == ca_chat || op == ca_secret_chat ||
        op == ca_peer || op == ca_number || op == ca_double ||
        op == ca_msg_id || op == ca_command || op == ca_channel) {
      if (cur_token_quoted) {
        if (opt) {
          line_ptr = save;
          flags++;
...
int complete_string_list(char **list, int index, const char *text, int len,
                         char **R) {
  index++;
...
int complete_spec_message_answer(struct tgl_message *M, int index,
                                 const char *text, int len, char **R) {
...
int complete_message_answer(tgl_peer_t *P, int index, const char *text, int len,
                            char **R) {
...
int complete_user_command(tgl_peer_t *P, int index, const char *text, int len,
                          char **R) {
  if (len <= 0 || *text != '/') {
  ...
  if (index >= U->bot_info->commands_num) {
    return U->bot_info->commands_num +
           complete_message_answer(P, index - U->bot_info->commands_num,
                                   text - 1, len + 1, R);
  }

  index++;
  while (index < U->bot_info->commands_num &&
         strncmp(U->bot_info->commands[index].command, text, len)) {
    index++;
  }
  if (index < U->bot_info->commands_num) {
    *R = NULL;
...
int complete_chat_command(tgl_peer_t *P, int index, const char *text, int len,
                          char **R) {
  if (len <= 0 || *text != '/') {
    return complete_message_answer(P, index, text, len, R);
  }
  ...
  for (i = 0; i < P->chat.user_list_size; i++) {
    struct tgl_user *U =
        (void *)tgl_peer_get(TLS, TGL_MK_USER(P->chat.user_list[i].user_id));
    if (!U) {
      continue;
    }
    if (!U->bot_info) {
    ...
    while (index - tot < U->bot_info->commands_num &&
           strncmp(U->bot_info->commands[index - tot].command, text, p)) {
      index++;
    }
    if (index - tot < U->bot_info->commands_num) {
      *R = NULL;
      if (U->username) {
        assert(asprintf(R, "/%s@%s", U->bot_info->commands[index].command,
                        U->username) >= 0);
      } else {
        assert(asprintf(R, "/%s", U->bot_info->commands[index].command) >= 0);
      }
  ...
  if (index == tot) {
    ...
    return tot +
           complete_message_answer(P, index - tot - 1, text - 1, len + 1, R);
  }
}

int complete_username(int mode, int index, const char *text, int len,
                      char **R) {
  *R = NULL;
  if (len > 0 && *text == '@') {
    text++;
...
char *command_generator(const char *text, int state) {
  ...
  if (!state) {
    ...
    if (mode != ca_file_name && mode != ca_file_name_end && index == -1) {
      return 0;
    }
  }

  if (mode == ca_none || mode == ca_string || mode == ca_string_end ||
      mode == ca_number || mode == ca_double || mode == ca_msg_id) {
    if (c) {
  ...
  case ca_user:
    if (command_len && command_pos[0] == '@') {
      index =
          complete_username(TGL_PEER_USER, index, command_pos, command_len, &R);
    } else {
  ...
  case ca_secret_chat:
    index =
        tgl_complete_encr_chat_list(TLS, index, command_pos, command_len, &R);
    if (c) {
      rl_line_buffer[rl_point] = c;
    }
    return R;
  case ca_channel:
    if (command_len && command_pos[0] == '@') {
      index = complete_username(TGL_PEER_CHANNEL, index, command_pos,
                                command_len, &R);
    } else {
      index =
          tgl_complete_channel_list(TLS, index, command_pos, command_len, &R);
    }
    if (c) {
      rl_line_buffer[rl_point] = c;
    }
  ...
  case ca_modifier:
    index =
        complete_string_list(modifiers, index, command_pos, command_len, &R);
    if (c) {
      rl_line_buffer[rl_point] = c;
    }
    return R;
  case ca_msg_string_end:
    if (autocomplete_peer) {
      if (tgl_get_peer_type(autocomplete_peer->id) == TGL_PEER_USER) {
        index = complete_user_command(autocomplete_peer, index, command_pos,
                                      command_len, &R);
      }
      if (tgl_get_peer_type(autocomplete_peer->id) == TGL_PEER_CHAT) {
        index = complete_chat_command(autocomplete_peer, index, command_pos,
                                      command_len, &R);
      }
    }
    if (autocomplete_id.peer_type != (unsigned)NOT_FOUND) {
      ...
      if (M) {
        if (command_len > 0 && *command_pos == '/') {
          ...
          index = complete_spec_message_answer(M, index, command_pos,
                                               command_len, &R);
        }
      }
    }
...
#ifndef DISABLE_EXTF
  case ca_extf:
    index = tglf_extf_autocomplete(TLS, text, len, index, &R, rl_line_buffer,
                                   rl_point);
    if (c) {
      rl_line_buffer[rl_point] = c;
    }
...
void print_fail(struct in_ev *ev) {
  ...
  if (!enable_json) {
    ...
    assert(json_object_set_new(res, "error_code",
                               json_integer(TLS->error_code)) >= 0);
    assert(json_object_set_new(res, "error",
                               json_string(TLS->error ? TLS->error : "")) >= 0);
...
void fail_interface(struct tgl_state *TLS, struct in_ev *ev, int error_code,
  ...
  if (!enable_json) {
    ...
    json_t *res = json_object();
    assert(json_object_set_new(res, "result", json_string("FAIL")) >= 0);
    assert(json_object_set_new(res, "error_code", json_integer(error_code)) >=
           0);
...
void print_msg_list_success_gw(struct tgl_state *TLSR, void *extra, int success,
                               int num, struct tgl_message *ML[]) {
  assert(TLS == TLSR);
...
void print_msg_list_gw(struct tgl_state *TLSR, void *extra, int success,
                       int num, struct tgl_message *ML[]) {
  assert(TLS == TLSR);
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
...
void print_user_gw(struct tgl_state *TLSR, void *extra, int success,
                   struct tgl_user *U) {
  assert(TLS == TLSR);
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
...
void print_filename_gw(struct tgl_state *TLSR, void *extra, int success,
                       const char *name) {
  assert(TLS == TLSR);
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
...
void print_chat_info_gw(struct tgl_state *TLSR, void *extra, int success,
                        struct tgl_chat *C) {
  assert(TLS == TLSR);
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
  ...
  if (!enable_json) {
    ...
    for (i = 0; i < C->user_list_size; i++) {
      mprintf(ev, "\t\t");
      print_user_name(ev, TGL_MK_USER(C->user_list[i].user_id),
                      tgl_peer_get(TLS, TGL_MK_USER(C->user_list[i].user_id)));
      mprintf(ev, " invited by ");
      print_user_name(
          ev, TGL_MK_USER(C->user_list[i].inviter_id),
          tgl_peer_get(TLS, TGL_MK_USER(C->user_list[i].inviter_id)));
      mprintf(ev, " at ");
...
void print_user_status(struct tgl_user_status *S, struct in_ev *ev) {
  assert(!enable_json); // calling functions print_user_info_gw() and
                        // user_status_upd() already check.
  if (S->online > 0) {
...
void print_user_info_gw(struct tgl_state *TLSR, void *extra, int success,
                        struct tgl_user *U) {
  assert(TLS == TLSR);
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
  ...
  if (!enable_json) {
    ...
    if (U->bot_info) {
      ...
      for (i = 0; i < U->bot_info->commands_num; i++) {
        mprintf(ev, "\t\t/%s: %s\n", U->bot_info->commands[i].command,
                U->bot_info->commands[i].description);
      }
    }
    mpop_color(ev);
  } else {
...
void print_dialog_list_gw(struct tgl_state *TLSR, void *extra, int success,
  ...
  if (!enable_json) {
    ...
    for (i = size - 1; i >= 0; i--) {
      ...
      assert(json_object_set_new(a, "unread", json_integer(unread_count[i])) >=
             0);
      assert(json_array_append_new(res, a) >= 0);
    }
    char *s = json_dumps(res, 0);
...
void interpreter_chat_mode(char *line) {
  ...
  if (!strncmp(line, "/history", 8)) {
    ...
    tgl_do_get_history(TLS, chat_mode_id, 0, limit, offline_mode,
                       print_msg_list_gw, 0);
    return;
  }
  ...
  for (i = 0; i < num; i++)
    if (list[i]) {
      ...
      if (!enable_json) {
        ...
        mprintf(ev,
                " marked read %d outbox and %d inbox messages -- [%d/%02d/%02d "
                "%02d:%02d:%02d]\n",
                c1, c2, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
void mark_read_upd(struct tgl_state *TLSR, int num,
  ...
  if (unread_message_count + num <= MAX_UNREAD_MESSAGE_COUNT) {
    memcpy(unread_message_list + unread_message_count, list,
           num * sizeof(void *));
...
void print_typing(struct in_ev *ev, enum tgl_typing_status status) {
  ...
  case tgl_typing_typing:
    mprintf(ev, "typing -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
    break;
  case tgl_typing_cancel:
    mprintf(ev, "deleting typed message -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_record_video:
    mprintf(ev, "recording video -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_upload_video:
    mprintf(ev, "uploading video -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_record_audio:
    mprintf(ev, "recording audio -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_upload_audio:
    mprintf(ev, "uploading audio -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_upload_photo:
    mprintf(ev, "uploading photo -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_upload_document:
    mprintf(ev, "uploading document -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_geo:
    mprintf(ev, "choosing location -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  case tgl_typing_choose_contact:
    mprintf(ev, "choosing contact -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
    break;
  }
...
void type_notification_upd(struct tgl_state *TLSR, struct tgl_user *U,
                           enum tgl_typing_status status) {
  assert(TLSR == TLS);
  if (log_level < 2 || (disable_output && !notify_ev)) {
    return;
  }
  if (enable_json) {
    return;
...
void type_in_chat_notification_upd(struct tgl_state *TLSR, struct tgl_user *U,
                                   struct tgl_chat *C,
                                   enum tgl_typing_status status) {
  assert(TLSR == TLS);
  if (log_level < 2 || (disable_output && !notify_ev)) {
...
void print_peer_updates(struct in_ev *ev, int flags) {
  ...
  if (flags & TGL_UPDATE_PHONE) {
    mprintf(ev, " phone -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_CONTACT) {
    mprintf(ev, " contact -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_PHOTO) {
    mprintf(ev, " photo -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_BLOCKED) {
    mprintf(ev, " blocked -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_REAL_NAME) {
    mprintf(ev, " name -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_NAME) {
    mprintf(ev, " contact_name -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
  }
  if (flags & TGL_UPDATE_REQUESTED) {
    mprintf(ev, " status -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  ...
  if (flags & TGL_UPDATE_FLAGS) {
    mprintf(ev, " flags -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_TITLE) {
    mprintf(ev, " title -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_ADMIN) {
    mprintf(ev, " admin -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_MEMBERS) {
    mprintf(ev, " members -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
  if (flags & TGL_UPDATE_ACCESS_HASH) {
    mprintf(ev, " access_hash -- [%d/%02d/%02d %02d:%02d:%02d]",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min,
            tm.tm_sec);
  }
  if (flags & TGL_UPDATE_USERNAME) {
    mprintf(ev, " username -- [%d/%02d/%02d %02d:%02d:%02d]", tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  }
}
...
void peer_update_username(tgl_peer_t *P, const char *username) {
  if (!username) {
    if (P->extra) {
      struct username_peer_pair *p =
          tree_lookup_username_peer_pair(username_peer_pair, (void *)&P->extra);
      assert(p);
      username_peer_pair =
          tree_delete_username_peer_pair(username_peer_pair, p);
      tfree_str(P->extra);
      tfree(p, sizeof(*p));
      P->extra = NULL;
    }
  ...
  username_peer_pair =
      tree_insert_username_peer_pair(username_peer_pair, p, rand());
}

void user_update_gw(struct tgl_state *TLSR, struct tgl_user *U,
                    unsigned flags) {
  assert(TLSR == TLS);
#ifdef USE_LUA
...
void chat_update_gw(struct tgl_state *TLSR, struct tgl_chat *U,
                    unsigned flags) {
  assert(TLSR == TLS);
#ifdef USE_LUA
...
void secret_chat_update_gw(struct tgl_state *TLSR, struct tgl_secret_chat *U,
                           unsigned flags) {
  assert(TLSR == TLS);
#ifdef USE_LUA
...
void channel_update_gw(struct tgl_state *TLSR, struct tgl_channel *U,
                       unsigned flags) {
  assert(TLSR == TLS);
...
void print_card_gw(struct tgl_state *TLSR, void *extra, int success, int size,
                   int *card) {
  assert(TLSR == TLS);
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
...
void callback_extf(struct tgl_state *TLS, void *extra, int success,
                   const char *buf) {
  struct in_ev *ev = extra;
  if (ev && !--ev->refcnt) {
    free(ev);
...
void do_get_values(struct tgl_state *TLS, enum tgl_value_type type,
                   const char *prompt, int num_values,
                   void (*callback)(struct tgl_state *TLS, const char *string[],
                                    void *arg),
                   void *arg);
...
void interpreter_ex(char *line, void *ex) {
  ...
  if (!command->name) {
    fail_interface(TLS, ex, ENOSYS, "can not find command '%.*s'",
                   cur_token_len, cur_token);
    in_readline = 0;
    return;
  }

  enum command_argument *flags = command->args;
  void (*fun)(struct command *, int, struct arg[], struct in_ev *) =
      command->fun;
  int args_num = 0;
  ...
  while (1) {
    ...
    if (op == ca_string_end || op == ca_file_name_end ||
      ...
      next_token_end();
      if (cur_token_len < 0) {
        fail_interface(TLS, ex, ENOSYS, "can not parse string_end arg #%d",
                       args_num);
        break;
      } else {
    ...
    if (op == ca_string || op == ca_file_name || op == ca_command) {
      if (cur_token_end_str || cur_token_len < 0) {
        ...
        fail_interface(TLS, ex, ENOSYS, "can not parse string arg #%d",
                       args_num);
        break;
      } else {
        args[args_num].flags = 1;
...
void interpreter(char *line) { interpreter_ex(line, 0); }
...
void print_media(struct in_ev *ev, struct tgl_message_media *M) {
  ...
  case tgl_message_media_geo:
    mprintf(ev, "[geo https://maps.google.com/?q=%.6lf,%.6lf]", M->geo.latitude,
            M->geo.longitude);
    return;
  ...
  case tgl_message_media_venue:
    mprintf(ev, "[geo https://maps.google.com/?q=%.6lf,%.6lf",
...
static char *monthes[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
void print_date(struct in_ev *ev, long t) {
  ...
  if (time(0) - t < 12 * 60 * 60) {
    ...
    mprintf(ev, "[%02d %s %d]", tm->tm_mday, monthes[tm->tm_mon],
            tm->tm_year + 1900);
  }
}
...
void print_date_full(struct in_ev *ev, long t) {
  struct tm *tm = localtime((void *)&t);
  mprintf(ev, "[%04d/%02d/%02d %02d:%02d:%02d]", tm->tm_year + 1900,
          tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
}

void print_msg_id(struct in_ev *ev, tgl_message_id_t msg_id,
                  struct tgl_message *M) {
  if (msg_num_mode) {
...
void print_service_message(struct in_ev *ev, struct tgl_message *M) {
  ...
  case tgl_message_action_chat_add_users:
    mprintf(ev, " added users:");
    {
      ...
      for (i = 0; i < M->action.user_num; i++) {
        print_user_name(ev, tgl_set_peer_id(TGL_PEER_USER, M->action.users[i]),
                        tgl_peer_get(TLS, tgl_set_peer_id(TGL_PEER_USER,
                                                          M->action.users[i])));
      }
    }
    mprintf(ev, "\n");
  ...
  case tgl_message_action_chat_add_user_by_link:
    ...
    print_user_name(
        ev, tgl_set_peer_id(TGL_PEER_USER, M->action.user),
        tgl_peer_get(TLS, tgl_set_peer_id(TGL_PEER_USER, M->action.user)));
    mprintf(ev, "\n");
    break;
